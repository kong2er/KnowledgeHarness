...
- 全量 pytest 自动化测试套件（当前为 5 份 stdlib-only 测试脚本：`tests/test_parse_inputs.py` / `test_stage1_core.py` / `test_topic_coarse_classify.py` / `test_phase2_features.py` / `test_phase3_non_api.py`，合计 70 条用例）
...
- 禁止把占位功能（高级 conflict detection、服务层 API、pytest 全量套件）描述为已实现
