...
- Flask 服务入口
- 全量 pytest 自动化测试套件（当前为 6 份 stdlib-only 测试脚本：`tests/test_parse_inputs.py` / `test_stage1_core.py` / `test_topic_coarse_classify.py` / `test_phase2_features.py` / `test_phase3_non_api.py` / `test_api_service_entry.py`，合计 70 条用例 + 1 条 FastAPI 入口断言）
