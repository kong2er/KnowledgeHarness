...
### 在 MVP 范围内
- txt / md / pdf / **docx** 解析
- 图片（`.png / .jpg / .jpeg`）**opt-in OCR**：本地依赖齐全时走本地 OCR；在 `api_assist` 显式开启且 API 可用时可选走图片 API OCR 增强；失败降级不崩溃
...
- 全量 pytest 自动化测试套件（当前为 9 份 stdlib-only 测试脚本：`tests/test_parse_inputs.py` / `test_stage1_core.py` / `test_topic_coarse_classify.py` / `test_phase2_features.py` / `test_phase3_non_api.py` / `test_api_service_entry.py` / `test_flask_service_entry.py` / `test_ui_server_port_fallback.py` / `test_simple_ui.py`，含可选依赖 SKIP 语义）
...
- [ ] 或执行 `./scripts/run_acceptance_gate.sh` 一键通过基础门禁（测试 + demo + 结果结构检查）
...
- [ ] 若属于阶段性收尾/发布前审计，已同步 `docs/ENGINEERING_REVIEW.md`（能力覆盖/缺陷/优化路线）
...
- [ ] 当 `api_assist` 显式开启且 API 已配置时，图片允许走可选 API OCR 增强（`fallback_only/auto/prefer_api`）；失败必须降级，不得中断主流程
...
- [ ] 若启用图片 API OCR 增强，`ingestion_summary` 需补充 `image_api_assist_enabled / image_api_enhance_mode / image_api_attempted / image_api_succeeded / image_api_enhanced`
...
- [ ] 可选 API 协助时，返回类别必须仍受 `CATEGORIES` 约束；越界标签必须拒绝并降级
- [ ] API 不可用/超时时不得中断主流程，需保留本地分类结果并记录 warnings
...
- [ ] 支持 `custom` / `openai_compatible` 协议风格（`auto` 可识别并路由）；openai-compatible 响应需能从 `choices[0].message.content` 中提取 JSON
...
- [ ] 可选 API 协助仅允许整理 `stage_3` 固定结构（四个列表字段），不得改写 stage_1/stage_2 契约
- [ ] API 不可用/超时时必须降级回本地 stage_3 结果，不中断主流程
...
### validate_result
- [ ] 必须支持 `validation_profile`（`strict/lenient`），默认 `strict`
- [ ] `strict` 必须检查：未分类比例（>35% 告警）、空主分类、重复 chunk、阶段总结缺失
- [ ] `lenient` 必须在小样本/OCR 场景放宽阈值，降低误报（如未分类比例阈值、空主分类触发条件）
...
- [ ] `/health.features` 需至少覆盖 `topic_api_configured / web_enrichment_api_configured / image_ocr_api_configured`
- [ ] 提供 `POST /pipeline/run` 并复用 `run_pipeline`（不得分叉核心逻辑）
- [ ] 输入无有效文件时返回 4xx（不允许静默成功）
- [ ] 服务层不得篡改降级语义（topic/web enrichment 失败仍需可降级）
- [ ] 运行参数必须通过共享解析层构建（`build_pipeline_run_kwargs`）；请求未提供字段时应继承 `config/pipeline_config.json`
...
- [ ] UI 首页必须采用“左操作 / 右状态结果”分区；不得回退为无层级堆叠控件页
- [ ] 一级主按钮必须唯一且突出（`开始整理` / 调试视图 `运行流水线`）；不得与“打开结果/设置”同权重
- [ ] 文件导入区必须同时提供“选择文件”和“选择文件夹”，并可见本次选择状态
- [ ] 状态区必须可见：当前阶段、成功数、失败数、空文本数、处理文件数
- [ ] 结果区必须可见：导出状态 + “打开结果文件” + “打开输出目录”；不可用时需禁用或给出明确说明
- [ ] 必须提供 `GET /outputs`（及调试视图等价入口）用于输出目录浏览；该页面不得绕过 `/download` 白名单约束
...
- [ ] `/run` 的参数组装需复用共享解析层（`build_pipeline_run_kwargs`），确保与 CLI/API 的配置继承行为一致
- [ ] 命令行直启 `service/simple_ui.py` 在端口冲突时应自动回退可用端口（默认向后尝试），并输出实际端口
- [ ] `tests/test_simple_ui.py` 需覆盖 HTTP 关键路径：settings 密钥不回显、下载白名单、上传限额/输入类 400、流水线异常 500（受限环境可 SKIP）
...
- [ ] 支持 `--validation-profile strict|lenient`，并可由共享运行时配置继承默认值
- [ ] CLI 运行参数需通过共享解析层（`build_pipeline_run_kwargs`）构建，避免与 API/UI 漂移
- [ ] API 协助默认关闭；仅在显式开启（CLI `--enable-api-assist` / UI 勾选 / 服务请求字段）时，`auto` 模式才允许调用外部 API
